...
After we have cleaned and analysed data, we can conclude which features we believe to be important in the model. It is considered a good practice to identify which features are important when building predictive models. Now, let's use an automated feature selection process called Boruta. It is a feature ranking and selection algorithm based on random forests algorithm. The advantage with Boruta is that it clearly decides if a variable is important or not and helps to select variables that are statistically significant. 

```{r}
boruta_output <- Boruta(loan_status ~ ., data = lending_loans, doTrace = 0, maxRuns = 100)  
```

Boruta performed 19 iterations in 33.35373 mins. 22 attributes were confirmed important and no attributes deemed unimportant.
```{r}
print(boruta_output)
```

We can also plot importance history. If the importance value falls within the blue lines that is more likely to be unimportant attribute. And in the green we have confirmed important attributes. 
Our graph has three blue lines with rest being green.
```{r}
plotImpHistory(boruta_output)
```
```{r}
# statistical significance decision column
attStats(boruta_output)
```
##### Data Spliting

Before building the model we need to split our data into training and test set. The best way to split our data into training and test set is by cross-validation method that uses multiple test sets. However, cross-validation method is very time consuming process. For this reason, in this project, I have decided to use single test set method where I split data-set into training set (70%) and test set (30%).
...
test %>%
  tabyl(loan_status)
```
Very good results, we have close percentage of both values in raining and testing sets.

We have carried out automated feature selection and did data analysis so we insert our predictors into the model.

##### Logistic regression

```{r}
# logistic regression with all predictors
logistic_regressor <- glm(loan_status ~ ., data = train, family = binomial(link = "logit"))
...
```{r}
roc_train <- train %>%
  add_predictions(logistic_regressor, type = "response") %>%
  roc(response = loan_status, predictor = pred)

auc(roc_train) # 0.9294
...
auc(roc_test) # 0.9271
```
Our model has high AUC.


Plotting the ROC-curve:
```{r}
all_loans_pred <- test %>% # ????? do we use whole datset or test?? in notes whole but why
  add_predictions(logistic_regressor, type = "response")

roc_all_pred <- all_loans_pred %>%
  roc(response = loan_status, predictor = pred)

roc_curve <- ggroc(data = roc_all_pred, legacy.axes = TRUE) +
  coord_fixed()
...
classifier_data <- tibble(
  threshold = roc_all_pred$thresholds,
  sensitivity = roc_all_pred$sensitivities,
  specificity = roc_all_pred$specificities
)
...
lending_loans_c <- lending_loans %>% 
  mutate(loan_status = as_factor(if_else(loan_status, "t", "f"))) %>%
  mutate(delinq_2yrs = as_factor(if_else(delinq_2yrs, "t", "f"))) %>% 
  mutate(inq_last_6mths = as_factor(if_else(inq_last_6mths, "t", "f"))) %>%
  mutate(pub_rec = as_factor(if_else(pub_rec, "t", "f"))) %>% 
  mutate(total_rec_late_fee = as_factor(if_else(total_rec_late_fee, "t", "f"))) %>%
  mutate(recoveries = as_factor(if_else(recoveries, "t", "f"))) %>% 
  mutate(term_36 = as_factor(if_else(term_36, "t", "f")))
...
loans_23 <- randomForest(loan_status ~ ., data = train)
```
This model is based on 23 variables. The number of trees in random forest is 500 and number of variables tried at each split is 7. This model explains 70.89%. 
```{r}
loans_23
# missing estimate error and confusion matrix
```

##### Predistions and Confustion Matrix - Test

```{r}

prediction_for_table <- predict(loans_23)
table(predicted=prediction_for_table)
library(modelr)

# add the predictions
test_pred <- test %>%
  add_predictions(lending_loans, type = "response")
...
library(caret)
set.seed(42)
inp <- sample(2, nrow(lending_loans), replace = T, prob = c(0.7, 0.3))
ttrain <- lending_loans[inp == 1,]
ttest <- lending_loans[inp == 2,]
pred <- predict(loans_23, ttest)



confusionMatrix(p, ttest$loan_status)
pred <- format(round(predict(loans_23,test)))            

confusionMatrix(table(factor(p, levels=min(test):max(test)),
                      factor(test, levels=min(test):max(test))))

a <- table(factor(pred, levels=min(test):max(test)), 
      factor(test, levels=min(test):max(test)))


loans_23 <- lending_loans %>% 
  add_predictions(loans_23, type = "response")

threshold <- 0.6 # set by us
loans_23 <- loans_23 %>% 
  mutate(pred_cut_off = pred >= threshold)

loans_23 %>% 
  tabyl(loan_status, pred_cut_off)
```