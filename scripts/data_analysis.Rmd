...
log_reg_model <- glm(loan_status ~ emp_length + purpose + pub_rec_bankruptcies +  annual_inc + inq_last_6mths + revol_bal + int_rate_pct + revol_util_pct + install_mth_pct + fico_range_avg + term_36, data = train, family = binomial)
...
# CONCLUSION
The two most important measures to evaluate models are accuracy and error values. 

The accuracy of our model is 70%. The model produces a large type II error with over 30% of the false-negative rate. It performs better predicting who is likely to pay the loan in comparison to predicting who is likely to default. The reason for this poor performance could be the unbalanced dataset. The model learned from the training set with a significantly higher proportion of fully paid compared to default loans. Therefore, default loan is a rare event in our training data set.
The classification accuracy could be a problematic measure for disproportionate classification which is a common scenario in the investment and banking sector and can lead us to make wrong decisions. 
...
For future model performance improvement, I would suggest applying additional methods such as strategy curve, cost-sensitive learning, under-sampling, or over-sampling. Also, other methods can be used for classification, for instance, random forest, neural networks, or naive Bayes. 
