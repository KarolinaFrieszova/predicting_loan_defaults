...
In dealing with missing values, variables which contained a lot of missing values were removed, along with 32 observations which were missing across multiple columns. Other missing values were replaced with median imputation. 
```{r}
...
Target variable in the model is loan_status. There are 9 different values. Here are the values explained:
...
lending_loans %>% 
  select(annual_inc, loan_amnt, purpose, loan_status) %>% 
...
loans_numeric$paid_loan <- lending_loans$paid_loan

names(loans_nonnumeric)
nonnumeric_1 <- loans_nonnumeric %>% 
  select(paid_loan, term, verification_status, out_prncp, pub_rec_bankruptcies)

ggpairs(nonnumeric_1)
```

```{r}
ggsave("nonnumerical.png")
```
